...
# Incluye: Documentos, Carnets, Asistencias QR, WhatsApp
# ========================================
...
class BaseDatos:
    ...
    def guardar_asistencia(dni, nombre, tipo, hora):
        """Guarda asistencia con hora exacta"""
        fecha_hoy = datetime.now().strftime('%Y-%m-%d')
        
        if Path(BaseDatos.ASISTENCIAS).exists():
            with open(BaseDatos.ASISTENCIAS, 'r', encoding='utf-8') as f:
        ...
        with open(BaseDatos.ASISTENCIAS, 'w', encoding='utf-8') as f:
...
# ========================================
# GENERADOR DE PDFs (CON FECHAS EN ESPAÑOL)
# ========================================
...
class GeneradorPDF:
    ...
    def _obtener_fecha_espanol(self):
        """MEJORADO: Fecha siempre en español"""
    ...
    def _agregar_solicitante(self, datos, y):
        texto_solicitud = (
            ...
            f"<b>{datos['apoderado'].upper()}</b> con DNI N° <b>{datos['dni_apo']}</b>."
        )
        
        estilo_solicitud = ParagraphStyle(
    ...
    def generar_resolucion_traslado(self, datos):
        ...
        self.canvas.drawCentredString(self.width/2, y, 
                                     f"RESOLUCIÓN DIRECTORAL N° {datos['num_resolucion']}")
        
        y -= 30
        self.canvas.setFont("Helvetica", 11)
        fecha_resolucion = datos.get('fecha_resolucion', self._obtener_fecha_espanol())
        self.canvas.drawCentredString(self.width/2, y, fecha_resolucion)
        
        y -= 40
        mx, ancho = 60, self.width - 120
        
        estilo_normal = ParagraphStyle('Normal', parent=self.styles['Normal'],
        ...
        texto_visto = (
            f"La solicitud del(a) apoderado(a), de <b>{datos['alumno'].upper()}</b> y el informe "
            f"de progreso de <b>{datos['nivel'].upper()}</b>."
        )
        ...
        texto_considerando = (
            ...
            "apoderados lo soliciten, a fin de garantizar la continuidad de estudios del educando."
        )
    ...
    def generar_carta_compromiso(self, datos):
        ...
        estilo_comp = ParagraphStyle('Compromiso', parent=self.styles['Normal'],
                                    fontSize=8.5, leading=11, alignment=TA_JUSTIFY)
        
        intro = (
            f"Yo, <b>{datos['apoderado'].upper()}</b>, con DNI N° <b>{datos['dni_apo']}</b>, "
            f"padre/madre/apoderado(a) de <b>{datos['alumno'].upper()}</b>, estudiante del "
            f"<b>{datos['grado'].upper()}</b>, me comprometo formalmente a cumplir las siguientes "
            f"obligaciones establecidas por la I.E. ALTERNATIVO YACHAY:"
        )
...
class GeneradorCarnet:
    """MEJORADO: Carnet con celular y diseño optimizado"""
    WIDTH = 1012
    HEIGHT = 638
    AZUL_INST = (0, 30, 120)
    DORADO = (255, 215, 0)
    ...
    def _aplicar_escudo_fondo(self):
        if Path("escudo_upload.png").exists():
            try:
                ...
                escudo = escudo.resize((350, 350), Image.LANCZOS)
                
                capa = Image.new('RGBA', (self.WIDTH, self.HEIGHT), (0, 0, 0, 0))
                x = int((self.WIDTH - 350) / 2)
                y = int((self.HEIGHT - 350) / 2)
                capa.paste(escudo, (x, y))
                
                datos_pixel = capa.getdata()
                nuevos_datos = [
                    (d[0], d[1], d[2], 25) if d[3] > 0 else d 
    ...
    def _dibujar_barras_decorativas(self):
        """MEJORADO: Barras con gradiente"""
        # Barra superior
        self.draw.rectangle([(0, 0), (self.WIDTH, 220)], fill=self.AZUL_INST)
        # Línea dorada decorativa
        self.draw.rectangle([(0, 215), (self.WIDTH, 220)], fill=self.DORADO)
        
        # Barra inferior
        self.draw.rectangle([(0, self.HEIGHT - 180), (self.WIDTH, self.HEIGHT)], fill=self.AZUL_INST)
        # Línea dorada decorativa
        self.draw.rectangle([(0, self.HEIGHT - 180), (self.WIDTH, self.HEIGHT - 175)], fill=self.DORADO)
    
    def _dibujar_textos_institucionales(self):
        # LETRAS GIGANTESCAS
        font_header = RecursoManager.obtener_fuente("Roboto-Bold.ttf", 200, bold=True)
        font_motto = RecursoManager.obtener_fuente("Roboto-Bold.ttf", 160, bold=True)
        
        self.draw.text((self.WIDTH/2, 110), "I.E. ALTERNATIVO YACHAY",
                      font=font_header, fill="white", anchor="mm")
        
        self.draw.text((self.WIDTH/2, self.HEIGHT - 90), "EDUCAR PARA LA VIDA",
                      font=font_motto, fill="white", anchor="mm")
    
    def _insertar_foto(self):
        x_foto, y_foto = 50, 240
        w_foto, h_foto = 290, 340
        ...
        # Marco dorado
        self.draw.rectangle([(x_foto-3, y_foto-3), (x_foto + w_foto+3, y_foto + h_foto+3)],
                          outline=self.DORADO, width=6)
    ...
    def _dibujar_datos_alumno(self):
        ...
        font_grado = font_data if len(grado_text) <= 12 else RecursoManager.obtener_fuente("Roboto-Regular.ttf", 50)
        self.draw.text((x_text + 220, y_grado), grado_text, font=font_grado, fill="black")
        
        # CELULAR (NUEVO)
        celular = self.datos.get('celular', '')
        if celular:
            self.draw.text((x_text, y_celular), "CEL:", font=font_label, fill="black")
            self.draw.text((x_text + 140, y_celular), celular, font=font_data, fill="black")
    ...
    def generar(self):
        ...
        self._dibujar_barras_decorativas()
...
def generar_mensaje_asistencia_individual(nombre, tipo, hora):
    """Genera mensaje individual de asistencia"""
...
def generar_reporte_whatsapp(asistencias):
    """NUEVO: Genera reporte completo de asistencias del día"""
    fecha = datetime.now().strftime('%d/%m/%Y')
    hora_reporte = datetime.now().strftime('%H:%M')
    
    mensaje = f"""📋 *REPORTE DE ASISTENCIAS*
...
"""
    ...
    con_entrada = sum(1 for a in asistencias.values() if a.get('entrada'))
    con_salida = sum(1 for a in asistencias.values() if a.get('salida'))
    
    mensaje += f"📊 *RESUMEN:*\n"
    mensaje += f"Total registrados: {total}\n"
    mensaje += f"✅ Con entrada: {con_entrada}\n"
    mensaje += f"🏁 Con salida: {con_salida}\n\n"
    
    mensaje += f"👥 *DETALLE:*\n"
    mensaje += "━━━━━━━━━━━━━━━\n"
    
    for dni, datos in sorted(asistencias.items(), key=lambda x: x[1]['nombre']):
        entrada = datos.get('entrada', '❌')
        salida = datos.get('salida', '❌')
        mensaje += f"\n• {datos['nombre']}\n"
        mensaje += f"  DNI: {dni}\n"
        mensaje += f"  Entrada: {entrada}\n"
        mensaje += f"  Salida: {salida}\n"
    
    mensaje += "\n━━━━━━━━━━━━━━━\n"
    mensaje += "✨ Reporte generado automáticamente por Sistema YACHAY PRO"
...
# ========================================
# TAB DE ASISTENCIAS CON ESCÁNER
# ========================================

def tab_asistencias():
    """TAB DE ASISTENCIAS con registro QR y reporte WhatsApp"""
    st.header("📋 Control de Asistencias")
    ...
    with col_tipo1:
        tipo_btn = "primary" if st.session_state.tipo_asistencia == "Entrada" else "secondary"
        if st.button("🌅 ENTRADA", use_container_width=True, type=tipo_btn, key=f"ent_{counter}"):
            st.session_state.tipo_asistencia = "Entrada"
            st.rerun()
    with col_tipo2:
        tipo_btn = "primary" if st.session_state.tipo_asistencia == "Salida" else "secondary"
        if st.button("🌙 SALIDA", use_container_width=True, type=tipo_btn, key=f"sal_{counter}"):
    ...
    with col_scan:
        ...
        st.markdown("### 📸 Escanear QR del Carnet")
        st.info("💡 **INSTRUCCIONES:**\n1. Abre la cámara de tu celular\n2. Escanea el QR del carnet\n3. Copia el DNI que aparece\n4. Pégalo en el campo de abajo")
        
        dni_escaneado = st.text_input("📱 DNI detectado:", key=f"dni_scan_{counter}",
                                       placeholder="Pega aquí el DNI escaneado")
        
        if st.button("✅ REGISTRAR ASISTENCIA", use_container_width=True, 
                    type="primary", key=f"reg_scan_{counter}"):
            if dni_escaneado:
                alumno = BaseDatos.buscar_por_dni(dni_escaneado)
                ...
                if alumno:
                    ...
                    if dni_escaneado not in [a['dni'] for a in st.session_state.asistencias_dia]:
                        st.session_state.asistencias_dia.append({
                            'dni': dni_escaneado,
                            'nombre': alumno['Alumno'],
                            'tipo': st.session_state.tipo_asistencia,
                            'hora': hora_exacta,
                            'celular': alumno.get('Celular', '')
                        })
                    
                    st.success(f"### ✅ {alumno['Alumno']}")
                    st.info(f"### 🕒 {hora_exacta}")
                    st.balloons()
                    
                    # Mensaje individual
                    mensaje_individual = generar_mensaje_asistencia_individual(
                        alumno['Alumno'],
                        st.session_state.tipo_asistencia.lower(),
                        hora_exacta
                    )
                    
                    st.text_area("📱 Mensaje:", mensaje_individual, height=120, key=f"msg_{counter}")
                    
                    # Celular guardado
                    celular_guardado = alumno.get('Celular', '')
                    if celular_guardado:
                        if st.button(f"📱 ENVIAR A {celular_guardado}", key=f"wa_{counter}"):
                            enviar_whatsapp(celular_guardado, mensaje_individual)
                            st.success("✅ Abriendo WhatsApp...")
                    else:
                        st.warning("⚠️ Este estudiante no tiene celular registrado")
                else:
    ...
    with col_manual:
        ...
        dni_manual = st.text_input("DNI:", key=f"dni_man_{counter}")
        
        if st.button("🔍 Buscar y Registrar", key=f"man_{counter}"):
            if dni_manual:
                ...
                if alumno:
                    ...
                    BaseDatos.guardar_asistencia(dni_manual, alumno['Alumno'], 
                                                st.session_state.tipo_asistencia.lower(), 
                                                hora_exacta)
                    if dni_manual not in [a['dni'] for a in st.session_state.asistencias_dia]:
                        st.session_state.asistencias_dia.append({
                            'dni': dni_manual,
                            'nombre': alumno['Alumno'],
                            'tipo': st.session_state.tipo_asistencia,
                    ...
                    st.success(f"✅ {alumno['Alumno']} - {hora_exacta}")
                else:
    ...
    if asistencias_hoy:
        ...
        with col_rep1:
            st.markdown("### 📱 Enviar Reporte Completo")
            telefono_reporte = st.text_input("Número para enviar reporte (ej: 51987654321):",
                                            key=f"tel_rep_{counter}")
        
        with col_rep2:
            st.markdown("### ")
            st.markdown("### ")
            if st.button("📨 ENVIAR REPORTE POR WHATSAPP", 
                        type="primary", use_container_width=True, key=f"rep_{counter}"):
                if telefono_reporte:
                    reporte = generar_reporte_whatsapp(asistencias_hoy)
                    enviar_whatsapp(telefono_reporte, reporte)
                    st.success("✅ Reporte enviado!")
                else:
                    st.error("⚠️ Ingresa un número de teléfono")
    else:
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            if tipo_doc == "CONSTANCIA DE CONDUCTA":
                ...
                num_resolucion = st.text_input("N° Resolución:", key="num_res")
                fecha_resolucion = st.text_input("Fecha:", value=datetime.now().strftime("Chinchero, %d de %B del %Y"), key="fec_res")
                nivel = st.selectbox("Nivel:", ["INICIAL", "PRIMARIA", "SECUNDARIA"], key="nivel_res")
                ie_destino = st.text_input("IE de Destino:", key="ie_dest")
                nivel_destino = st.text_input("Nivel de Continuidad:", key="niv_dest")
            ...
            if nombre and dni and apoderado and dni_apo:
                with st.spinner("Generando..."):
                    datos = {'alumno': nombre, 'dni': dni, 'grado': grado, 
                            'apoderado': apoderado, 'dni_apo': dni_apo, **notas_conducta}
                    
                    if tipo_doc == "RESOLUCIÓN DE TRASLADO":
                        datos.update({'num_resolucion': num_resolucion, 
                                    'fecha_resolucion': fecha_resolucion,
                                    'nivel': nivel, 'ie_destino': ie_destino, 
                    ...
                    nombre_archivo = f"{nombre.replace(' ', '_')}_{tipo_doc.replace(' ', '_')}.pdf"
                    st.download_button("⬇️ DESCARGAR", pdf, nombre_archivo, 
...
def tab_carnets(config):
    ...
    with col_individual:
        ...
        with st.container(border=True):
            ...
            i_cel = st.text_input("📱 Celular (opcional):", key="i_cel")
            ...
            if st.button("👁️ GENERAR", type="primary", use_container_width=True, key="btn_gen_ind"):
                if i_nom and i_dni:
                    with st.spinner("Generando..."):
                        ...
                        nombre_archivo = f"Carnet_{i_nom.replace(' ', '_')}.png"
                        st.download_button("⬇️ DESCARGAR", carnet, nombre_archivo, 
    ...
    with col_lote:
        ...
        with st.expander("➕ Agregar", expanded=True):
            ...
            if st.button("Buscar", key=f"btn_bus_{st.session_state.busqueda_counter}"):
                ...
                if resultado:
                    ...
                    st.session_state.c_temp_cel = resultado.get('Celular', '')
                    st.success("✅ Encontrado")
                    st.rerun()
                else:
                    st.warning("No encontrado")
            
            c_nom = st.text_input("Nombre:", value=st.session_state.get('c_temp_nom', ''), 
                                 key=f"c_nom_{st.session_state.busqueda_counter}")
            c_dni = st.text_input("DNI:", value=st.session_state.get('c_temp_dni', ''), 
                                 key=f"c_dni_{st.session_state.busqueda_counter}")
            c_gra = st.text_input("Grado:", value=st.session_state.get('c_temp_gra', ''), 
                                 key=f"c_gra_{st.session_state.busqueda_counter}")
            c_cel = st.text_input("Celular:", value=st.session_state.get('c_temp_cel', ''), 
                                 key=f"c_cel_{st.session_state.busqueda_counter}")
            c_foto = st.file_uploader("Foto:", type=['jpg', 'png', 'jpeg'], 
                                     key=f"c_foto_{st.session_state.busqueda_counter}")
            
            if st.button("➕ AGREGAR", use_container_width=True, 
                        key=f"btn_agr_{st.session_state.busqueda_counter}"):
                if c_nom and c_dni:
        ...
        if cantidad > 0:
            df_carrito = pd.DataFrame([
                {'Alumno': item['alumno'], 'DNI': item['dni'], 
                 'Grado': item['grado'], 'Celular': item.get('celular', 'N/A')}
            ...
            with col_desc:
                if st.button("🚀 DESCARGAR ZIP", type="primary", use_container_width=True, 
                           key="btn_desc_zip"):
                    with st.spinner("Generando..."):
                        ...
                        with zipfile.ZipFile(buffer_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                            for i, item in enumerate(st.session_state.cola_carnets):
                                ...
                                nombre_archivo = f"Carnet_{item['alumno'].replace(' ', '_')}.png"
                                zf.writestr(nombre_archivo, carnet.getvalue())
                                progreso.progress((i + 1) / cantidad)
                        
                        buffer_zip.seek(0)
                        st.balloons()
                        
                        st.download_button("⬇️ GUARDAR ZIP", buffer_zip, 
    ...
    if df is not None:
        ...
        with col4:
            if 'Celular' in df.columns:
                st.metric("📱 Con Celular", df['Celular'].notna().sum())
